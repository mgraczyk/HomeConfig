...
CommitSequence = namedtuple("CommitSequence", ["startTime","commits"])
...
def get_sequence(data, eventName):
    ...
    retval = CommitSequence(minTime, np.cumsum(seq))
    return retval
...
def plot_sequence(data, eventName, displayName):
    ...
    x = np.array(range(seq.startTime, seq.startTime + len(seq.commits)))
    y = seq.commits
...
def get_pktid_data(path):
    ...
    #plot_sequence(data, "250", "VStore_Miss")
